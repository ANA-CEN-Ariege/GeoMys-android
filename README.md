...
- **Filtrage CRUVED** : la liste des protocoles affichés ne contient que ceux sur lesquels l'utilisateur authentifié a au moins un droit > 0 (parité gn_mobile_monitoring) — le bloc `cruved` retourné par `/api/monitorings/modules` est exploité côté client.
