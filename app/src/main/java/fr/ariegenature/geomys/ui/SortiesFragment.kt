...
class SortiesFragment : Fragment() {
    ...
    private val envoiGroupe by lazy {
        EnvoiGroupeUi<Sortie>(
            fragment = this,
            vues = { _binding?.let { EnvoiGroupeUi.Vues(it.btnToutEnvoyer, it.progressEnvoi, it.tvMessageEnvoi) } },
            autorise = { GeoNatureConfig(requireContext()).occtaxPeutCreer },
            connexionConfiguree = { GeoNatureConfig(requireContext()).connexionConfiguree },
            chargerToutes = { sortieStore.charger() },
            envoyables = { sortiesEnvoyables(it) },
            envoyerUne = { item, cfg -> envoyerSortieVersGeoNature(item, sortieStore, cfg).let { it.succes to it.message } },
            rafraichir = { refreshList() },
        )
    }
    private fun majBoutonToutEnvoyer(toutes: List<Sortie>) = envoiGroupe.majBouton(ongletCourant == 0, toutes)
